"""Throwaway fixture module (34/78) -- not real code, see ../README.md.
...
def normalize_csv_writer_two(value):
...
def chunk_csv_writer_two(items, size):
...
def summarize_csv_writer_two(numbers):
...
def dedupe_preserving_order_csv_writer_two(items):
...
def flatten_one_level_csv_writer_two(nested):
...
def running_totals_csv_writer_two(numbers):
...
def clamp_csv_writer_two(value, low, high):
...
class CsvWriterTwoHelper:
    ...
    def __init__(self, label="csv_writer_two"):
        self.label = label
        self._history = []
