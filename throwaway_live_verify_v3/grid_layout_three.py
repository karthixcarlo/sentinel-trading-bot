"""Throwaway fixture module (74/78) -- not real code, see ../README.md.
...
def normalize_grid_layout_three(value):
...
def chunk_grid_layout_three(items, size):
...
def summarize_grid_layout_three(numbers):
...
def dedupe_preserving_order_grid_layout_three(items):
...
def flatten_one_level_grid_layout_three(nested):
...
def running_totals_grid_layout_three(numbers):
...
def clamp_grid_layout_three(value, low, high):
...
class GridLayoutThreeHelper:
    ...
    def __init__(self, label="grid_layout_three"):
        self.label = label
        self._history = []
