"""Throwaway fixture module (16/78) -- not real code, see ../README.md.
...
def normalize_color_mix_one(value):
...
def chunk_color_mix_one(items, size):
...
def summarize_color_mix_one(numbers):
...
def dedupe_preserving_order_color_mix_one(items):
...
def flatten_one_level_color_mix_one(nested):
...
def running_totals_color_mix_one(numbers):
...
def clamp_color_mix_one(value, low, high):
...
class ColorMixOneHelper:
    ...
    def __init__(self, label="color_mix_one"):
        self.label = label
        self._history = []
