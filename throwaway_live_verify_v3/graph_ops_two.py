"""Throwaway fixture module (47/78) -- not real code, see ../README.md.
...
def normalize_graph_ops_two(value):
...
def chunk_graph_ops_two(items, size):
...
def summarize_graph_ops_two(numbers):
...
def dedupe_preserving_order_graph_ops_two(items):
...
def flatten_one_level_graph_ops_two(nested):
...
def running_totals_graph_ops_two(numbers):
...
def clamp_graph_ops_two(value, low, high):
...
class GraphOpsTwoHelper:
    ...
    def __init__(self, label="graph_ops_two"):
        self.label = label
        self._history = []
