"""Throwaway fixture module (31/78) -- not real code, see ../README.md.
...
def normalize_string_builder_two(value):
...
def chunk_string_builder_two(items, size):
...
def summarize_string_builder_two(numbers):
...
def dedupe_preserving_order_string_builder_two(items):
...
def flatten_one_level_string_builder_two(nested):
...
def running_totals_string_builder_two(numbers):
...
def clamp_string_builder_two(value, low, high):
...
class StringBuilderTwoHelper:
    ...
    def __init__(self, label="string_builder_two"):
        self.label = label
        self._history = []
