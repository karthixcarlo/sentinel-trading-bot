"""Throwaway fixture module (30/78) -- not real code, see ../README.md.
...
def normalize_number_utils_two(value):
...
def chunk_number_utils_two(items, size):
...
def summarize_number_utils_two(numbers):
...
def dedupe_preserving_order_number_utils_two(items):
...
def flatten_one_level_number_utils_two(nested):
...
def running_totals_number_utils_two(numbers):
...
def clamp_number_utils_two(value, low, high):
...
class NumberUtilsTwoHelper:
    ...
    def __init__(self, label="number_utils_two"):
        self.label = label
        self._history = []
