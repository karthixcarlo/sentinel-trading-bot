"""Throwaway fixture module (68/78) -- not real code, see ../README.md.
...
def normalize_color_mix_three(value):
...
def chunk_color_mix_three(items, size):
...
def summarize_color_mix_three(numbers):
...
def dedupe_preserving_order_color_mix_three(items):
...
def flatten_one_level_color_mix_three(nested):
...
def running_totals_color_mix_three(numbers):
...
def clamp_color_mix_three(value, low, high):
...
class ColorMixThreeHelper:
    ...
    def __init__(self, label="color_mix_three"):
        self.label = label
        self._history = []
