"""Throwaway fixture module (8/78) -- not real code, see ../README.md.
...
def normalize_csv_writer_one(value):
...
def chunk_csv_writer_one(items, size):
...
def summarize_csv_writer_one(numbers):
...
def dedupe_preserving_order_csv_writer_one(items):
...
def flatten_one_level_csv_writer_one(nested):
...
def running_totals_csv_writer_one(numbers):
...
def clamp_csv_writer_one(value, low, high):
...
class CsvWriterOneHelper:
    ...
    def __init__(self, label="csv_writer_one"):
        self.label = label
        self._history = []
