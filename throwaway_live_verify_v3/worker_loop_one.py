"""Throwaway fixture module (13/78) -- not real code, see ../README.md.
...
def normalize_worker_loop_one(value):
...
def chunk_worker_loop_one(items, size):
...
def summarize_worker_loop_one(numbers):
...
def dedupe_preserving_order_worker_loop_one(items):
...
def flatten_one_level_worker_loop_one(nested):
...
def running_totals_worker_loop_one(numbers):
...
def clamp_worker_loop_one(value, low, high):
...
class WorkerLoopOneHelper:
    ...
    def __init__(self, label="worker_loop_one"):
        self.label = label
        self._history = []
