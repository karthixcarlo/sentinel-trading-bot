"""Throwaway fixture module (72/78) -- not real code, see ../README.md.
...
def normalize_tree_walk_three(value):
...
def chunk_tree_walk_three(items, size):
...
def summarize_tree_walk_three(numbers):
...
def dedupe_preserving_order_tree_walk_three(items):
...
def flatten_one_level_tree_walk_three(nested):
...
def running_totals_tree_walk_three(numbers):
...
def clamp_tree_walk_three(value, low, high):
...
class TreeWalkThreeHelper:
    ...
    def __init__(self, label="tree_walk_three"):
        self.label = label
        self._history = []
