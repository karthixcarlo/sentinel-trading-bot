"""Throwaway fixture module (52/78) -- not real code, see ../README.md.
...
def normalize_histogram_bins_two(value):
...
def chunk_histogram_bins_two(items, size):
...
def summarize_histogram_bins_two(numbers):
...
def dedupe_preserving_order_histogram_bins_two(items):
...
def flatten_one_level_histogram_bins_two(nested):
...
def running_totals_histogram_bins_two(numbers):
...
def clamp_histogram_bins_two(value, low, high):
...
class HistogramBinsTwoHelper:
    ...
    def __init__(self, label="histogram_bins_two"):
        self.label = label
        self._history = []
