"""Throwaway fixture module (5/78) -- not real code, see ../README.md.
...
def normalize_string_builder_one(value):
...
def chunk_string_builder_one(items, size):
...
def summarize_string_builder_one(numbers):
...
def dedupe_preserving_order_string_builder_one(items):
...
def flatten_one_level_string_builder_one(nested):
...
def running_totals_string_builder_one(numbers):
...
def clamp_string_builder_one(value, low, high):
...
class StringBuilderOneHelper:
    ...
    def __init__(self, label="string_builder_one"):
        self.label = label
        self._history = []
