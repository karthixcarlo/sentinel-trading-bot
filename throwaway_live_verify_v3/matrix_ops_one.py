"""Throwaway fixture module (24/78) -- not real code, see ../README.md.
...
def normalize_matrix_ops_one(value):
...
def chunk_matrix_ops_one(items, size):
...
def summarize_matrix_ops_one(numbers):
...
def dedupe_preserving_order_matrix_ops_one(items):
...
def flatten_one_level_matrix_ops_one(nested):
...
def running_totals_matrix_ops_one(numbers):
...
def clamp_matrix_ops_one(value, low, high):
...
class MatrixOpsOneHelper:
    ...
    def __init__(self, label="matrix_ops_one"):
        self.label = label
        self._history = []
