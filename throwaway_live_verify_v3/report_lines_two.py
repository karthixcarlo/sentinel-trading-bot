"""Throwaway fixture module (32/78) -- not real code, see ../README.md.
...
def normalize_report_lines_two(value):
...
def chunk_report_lines_two(items, size):
...
def summarize_report_lines_two(numbers):
...
def dedupe_preserving_order_report_lines_two(items):
...
def flatten_one_level_report_lines_two(nested):
...
def running_totals_report_lines_two(numbers):
...
def clamp_report_lines_two(value, low, high):
...
class ReportLinesTwoHelper:
    ...
    def __init__(self, label="report_lines_two"):
        self.label = label
        self._history = []
