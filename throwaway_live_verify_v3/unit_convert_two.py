"""Throwaway fixture module (44/78) -- not real code, see ../README.md.
...
def normalize_unit_convert_two(value):
...
def chunk_unit_convert_two(items, size):
...
def summarize_unit_convert_two(numbers):
...
def dedupe_preserving_order_unit_convert_two(items):
...
def flatten_one_level_unit_convert_two(nested):
...
def running_totals_unit_convert_two(numbers):
...
def clamp_unit_convert_two(value, low, high):
...
class UnitConvertTwoHelper:
    ...
    def __init__(self, label="unit_convert_two"):
        self.label = label
        self._history = []
