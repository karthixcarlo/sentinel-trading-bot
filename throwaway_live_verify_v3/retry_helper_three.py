"""Throwaway fixture module (62/78) -- not real code, see ../README.md.
...
def normalize_retry_helper_three(value):
...
def chunk_retry_helper_three(items, size):
...
def summarize_retry_helper_three(numbers):
...
def dedupe_preserving_order_retry_helper_three(items):
...
def flatten_one_level_retry_helper_three(nested):
...
def running_totals_retry_helper_three(numbers):
...
def clamp_retry_helper_three(value, low, high):
...
class RetryHelperThreeHelper:
    ...
    def __init__(self, label="retry_helper_three"):
        self.label = label
        self._history = []
