"""Throwaway fixture module (2/78) -- not real code, see ../README.md.
...
def normalize_list_helpers_one(value):
...
def chunk_list_helpers_one(items, size):
...
def summarize_list_helpers_one(numbers):
...
def dedupe_preserving_order_list_helpers_one(items):
...
def flatten_one_level_list_helpers_one(nested):
...
def running_totals_list_helpers_one(numbers):
...
def clamp_list_helpers_one(value, low, high):
...
class ListHelpersOneHelper:
    ...
    def __init__(self, label="list_helpers_one"):
        self.label = label
        self._history = []
