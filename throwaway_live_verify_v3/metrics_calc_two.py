"""Throwaway fixture module (41/78) -- not real code, see ../README.md.
...
def normalize_metrics_calc_two(value):
...
def chunk_metrics_calc_two(items, size):
...
def summarize_metrics_calc_two(numbers):
...
def dedupe_preserving_order_metrics_calc_two(items):
...
def flatten_one_level_metrics_calc_two(nested):
...
def running_totals_metrics_calc_two(numbers):
...
def clamp_metrics_calc_two(value, low, high):
...
class MetricsCalcTwoHelper:
    ...
    def __init__(self, label="metrics_calc_two"):
        self.label = label
        self._history = []
