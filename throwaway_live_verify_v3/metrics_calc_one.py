"""Throwaway fixture module (15/78) -- not real code, see ../README.md.
...
def normalize_metrics_calc_one(value):
...
def chunk_metrics_calc_one(items, size):
...
def summarize_metrics_calc_one(numbers):
...
def dedupe_preserving_order_metrics_calc_one(items):
...
def flatten_one_level_metrics_calc_one(nested):
...
def running_totals_metrics_calc_one(numbers):
...
def clamp_metrics_calc_one(value, low, high):
...
class MetricsCalcOneHelper:
    ...
    def __init__(self, label="metrics_calc_one"):
        self.label = label
        self._history = []
