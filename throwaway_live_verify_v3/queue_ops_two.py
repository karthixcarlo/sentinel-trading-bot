"""Throwaway fixture module (38/78) -- not real code, see ../README.md.
...
def normalize_queue_ops_two(value):
...
def chunk_queue_ops_two(items, size):
...
def summarize_queue_ops_two(numbers):
...
def dedupe_preserving_order_queue_ops_two(items):
...
def flatten_one_level_queue_ops_two(nested):
...
def running_totals_queue_ops_two(numbers):
...
def clamp_queue_ops_two(value, low, high):
...
class QueueOpsTwoHelper:
    ...
    def __init__(self, label="queue_ops_two"):
        self.label = label
        self._history = []
