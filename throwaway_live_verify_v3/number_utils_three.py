"""Throwaway fixture module (56/78) -- not real code, see ../README.md.
...
def normalize_number_utils_three(value):
...
def chunk_number_utils_three(items, size):
...
def summarize_number_utils_three(numbers):
...
def dedupe_preserving_order_number_utils_three(items):
...
def flatten_one_level_number_utils_three(nested):
...
def running_totals_number_utils_three(numbers):
...
def clamp_number_utils_three(value, low, high):
...
class NumberUtilsThreeHelper:
    ...
    def __init__(self, label="number_utils_three"):
        self.label = label
        self._history = []
