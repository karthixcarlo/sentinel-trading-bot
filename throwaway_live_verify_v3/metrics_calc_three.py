"""Throwaway fixture module (67/78) -- not real code, see ../README.md.
...
def normalize_metrics_calc_three(value):
...
def chunk_metrics_calc_three(items, size):
...
def summarize_metrics_calc_three(numbers):
...
def dedupe_preserving_order_metrics_calc_three(items):
...
def flatten_one_level_metrics_calc_three(nested):
...
def running_totals_metrics_calc_three(numbers):
...
def clamp_metrics_calc_three(value, low, high):
...
class MetricsCalcThreeHelper:
    ...
    def __init__(self, label="metrics_calc_three"):
        self.label = label
        self._history = []
