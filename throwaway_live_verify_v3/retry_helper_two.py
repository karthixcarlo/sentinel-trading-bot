"""Throwaway fixture module (36/78) -- not real code, see ../README.md.
...
def normalize_retry_helper_two(value):
...
def chunk_retry_helper_two(items, size):
...
def summarize_retry_helper_two(numbers):
...
def dedupe_preserving_order_retry_helper_two(items):
...
def flatten_one_level_retry_helper_two(nested):
...
def running_totals_retry_helper_two(numbers):
...
def clamp_retry_helper_two(value, low, high):
...
class RetryHelperTwoHelper:
    ...
    def __init__(self, label="retry_helper_two"):
        self.label = label
        self._history = []
