"""Throwaway fixture module (4/78) -- not real code, see ../README.md.
...
def normalize_number_utils_one(value):
...
def chunk_number_utils_one(items, size):
...
def summarize_number_utils_one(numbers):
...
def dedupe_preserving_order_number_utils_one(items):
...
def flatten_one_level_number_utils_one(nested):
...
def running_totals_number_utils_one(numbers):
...
def clamp_number_utils_one(value, low, high):
...
class NumberUtilsOneHelper:
    ...
    def __init__(self, label="number_utils_one"):
        self.label = label
        self._history = []
