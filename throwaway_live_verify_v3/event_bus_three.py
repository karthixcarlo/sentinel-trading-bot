"""Throwaway fixture module (66/78) -- not real code, see ../README.md.
...
def normalize_event_bus_three(value):
...
def chunk_event_bus_three(items, size):
...
def summarize_event_bus_three(numbers):
...
def dedupe_preserving_order_event_bus_three(items):
...
def flatten_one_level_event_bus_three(nested):
...
def running_totals_event_bus_three(numbers):
...
def clamp_event_bus_three(value, low, high):
...
class EventBusThreeHelper:
    ...
    def __init__(self, label="event_bus_three"):
        self.label = label
        self._history = []
