"""Throwaway fixture module (40/78) -- not real code, see ../README.md.
...
def normalize_event_bus_two(value):
...
def chunk_event_bus_two(items, size):
...
def summarize_event_bus_two(numbers):
...
def dedupe_preserving_order_event_bus_two(items):
...
def flatten_one_level_event_bus_two(nested):
...
def running_totals_event_bus_two(numbers):
...
def clamp_event_bus_two(value, low, high):
...
class EventBusTwoHelper:
    ...
    def __init__(self, label="event_bus_two"):
        self.label = label
        self._history = []
