"""Throwaway fixture module (39/78) -- not real code, see ../README.md.
...
def normalize_worker_loop_two(value):
...
def chunk_worker_loop_two(items, size):
...
def summarize_worker_loop_two(numbers):
...
def dedupe_preserving_order_worker_loop_two(items):
...
def flatten_one_level_worker_loop_two(nested):
...
def running_totals_worker_loop_two(numbers):
...
def clamp_worker_loop_two(value, low, high):
...
class WorkerLoopTwoHelper:
    ...
    def __init__(self, label="worker_loop_two"):
        self.label = label
        self._history = []
