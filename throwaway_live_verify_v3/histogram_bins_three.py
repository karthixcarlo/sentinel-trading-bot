"""Throwaway fixture module (78/78) -- not real code, see ../README.md.
...
def normalize_histogram_bins_three(value):
...
def chunk_histogram_bins_three(items, size):
...
def summarize_histogram_bins_three(numbers):
...
def dedupe_preserving_order_histogram_bins_three(items):
...
def flatten_one_level_histogram_bins_three(nested):
...
def running_totals_histogram_bins_three(numbers):
...
def clamp_histogram_bins_three(value, low, high):
...
class HistogramBinsThreeHelper:
    ...
    def __init__(self, label="histogram_bins_three"):
        self.label = label
        self._history = []
