"""Throwaway fixture module (42/78) -- not real code, see ../README.md.
...
def normalize_color_mix_two(value):
...
def chunk_color_mix_two(items, size):
...
def summarize_color_mix_two(numbers):
...
def dedupe_preserving_order_color_mix_two(items):
...
def flatten_one_level_color_mix_two(nested):
...
def running_totals_color_mix_two(numbers):
...
def clamp_color_mix_two(value, low, high):
...
class ColorMixTwoHelper:
    ...
    def __init__(self, label="color_mix_two"):
        self.label = label
        self._history = []
