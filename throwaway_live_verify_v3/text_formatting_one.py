"""Throwaway fixture module (1/78) -- not real code, see ../README.md.
...
def normalize_text_formatting_one(value):
...
def chunk_text_formatting_one(items, size):
...
def summarize_text_formatting_one(numbers):
...
def dedupe_preserving_order_text_formatting_one(items):
...
def flatten_one_level_text_formatting_one(nested):
...
def running_totals_text_formatting_one(numbers):
...
def clamp_text_formatting_one(value, low, high):
...
class TextFormattingOneHelper:
    ...
    def __init__(self, label="text_formatting_one"):
        self.label = label
        self._history = []
