"""Throwaway fixture module (3/78) -- not real code, see ../README.md.
...
def normalize_date_math_one(value):
...
def chunk_date_math_one(items, size):
...
def summarize_date_math_one(numbers):
...
def dedupe_preserving_order_date_math_one(items):
...
def flatten_one_level_date_math_one(nested):
...
def running_totals_date_math_one(numbers):
...
def clamp_date_math_one(value, low, high):
...
class DateMathOneHelper:
    ...
    def __init__(self, label="date_math_one"):
        self.label = label
        self._history = []
