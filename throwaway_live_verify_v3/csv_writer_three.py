"""Throwaway fixture module (60/78) -- not real code, see ../README.md.
...
def normalize_csv_writer_three(value):
...
def chunk_csv_writer_three(items, size):
...
def summarize_csv_writer_three(numbers):
...
def dedupe_preserving_order_csv_writer_three(items):
...
def flatten_one_level_csv_writer_three(nested):
...
def running_totals_csv_writer_three(numbers):
...
def clamp_csv_writer_three(value, low, high):
...
class CsvWriterThreeHelper:
    ...
    def __init__(self, label="csv_writer_three"):
        self.label = label
        self._history = []
