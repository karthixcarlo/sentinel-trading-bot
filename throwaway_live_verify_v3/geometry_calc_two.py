"""Throwaway fixture module (43/78) -- not real code, see ../README.md.
...
def normalize_geometry_calc_two(value):
...
def chunk_geometry_calc_two(items, size):
...
def summarize_geometry_calc_two(numbers):
...
def dedupe_preserving_order_geometry_calc_two(items):
...
def flatten_one_level_geometry_calc_two(nested):
...
def running_totals_geometry_calc_two(numbers):
...
def clamp_geometry_calc_two(value, low, high):
...
class GeometryCalcTwoHelper:
    ...
    def __init__(self, label="geometry_calc_two"):
        self.label = label
        self._history = []
