"""Throwaway fixture module (65/78) -- not real code, see ../README.md.
...
def normalize_worker_loop_three(value):
...
def chunk_worker_loop_three(items, size):
...
def summarize_worker_loop_three(numbers):
...
def dedupe_preserving_order_worker_loop_three(items):
...
def flatten_one_level_worker_loop_three(nested):
...
def running_totals_worker_loop_three(numbers):
...
def clamp_worker_loop_three(value, low, high):
...
class WorkerLoopThreeHelper:
    ...
    def __init__(self, label="worker_loop_three"):
        self.label = label
        self._history = []
