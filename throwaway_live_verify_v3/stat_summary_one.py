"""Throwaway fixture module (25/78) -- not real code, see ../README.md.
...
def normalize_stat_summary_one(value):
...
def chunk_stat_summary_one(items, size):
...
def summarize_stat_summary_one(numbers):
...
def dedupe_preserving_order_stat_summary_one(items):
...
def flatten_one_level_stat_summary_one(nested):
...
def running_totals_stat_summary_one(numbers):
...
def clamp_stat_summary_one(value, low, high):
...
class StatSummaryOneHelper:
    ...
    def __init__(self, label="stat_summary_one"):
        self.label = label
        self._history = []
