"""Throwaway fixture module (28/78) -- not real code, see ../README.md.
...
def normalize_list_helpers_two(value):
...
def chunk_list_helpers_two(items, size):
...
def summarize_list_helpers_two(numbers):
...
def dedupe_preserving_order_list_helpers_two(items):
...
def flatten_one_level_list_helpers_two(nested):
...
def running_totals_list_helpers_two(numbers):
...
def clamp_list_helpers_two(value, low, high):
...
class ListHelpersTwoHelper:
    ...
    def __init__(self, label="list_helpers_two"):
        self.label = label
        self._history = []
