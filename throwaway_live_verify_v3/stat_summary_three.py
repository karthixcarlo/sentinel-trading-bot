"""Throwaway fixture module (77/78) -- not real code, see ../README.md.
...
def normalize_stat_summary_three(value):
...
def chunk_stat_summary_three(items, size):
...
def summarize_stat_summary_three(numbers):
...
def dedupe_preserving_order_stat_summary_three(items):
...
def flatten_one_level_stat_summary_three(nested):
...
def running_totals_stat_summary_three(numbers):
...
def clamp_stat_summary_three(value, low, high):
...
class StatSummaryThreeHelper:
    ...
    def __init__(self, label="stat_summary_three"):
        self.label = label
        self._history = []
