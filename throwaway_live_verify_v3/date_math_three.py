"""Throwaway fixture module (55/78) -- not real code, see ../README.md.
...
def normalize_date_math_three(value):
...
def chunk_date_math_three(items, size):
...
def summarize_date_math_three(numbers):
...
def dedupe_preserving_order_date_math_three(items):
...
def flatten_one_level_date_math_three(nested):
...
def running_totals_date_math_three(numbers):
...
def clamp_date_math_three(value, low, high):
...
class DateMathThreeHelper:
    ...
    def __init__(self, label="date_math_three"):
        self.label = label
        self._history = []
