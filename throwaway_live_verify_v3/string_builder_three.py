"""Throwaway fixture module (57/78) -- not real code, see ../README.md.
...
def normalize_string_builder_three(value):
...
def chunk_string_builder_three(items, size):
...
def summarize_string_builder_three(numbers):
...
def dedupe_preserving_order_string_builder_three(items):
...
def flatten_one_level_string_builder_three(nested):
...
def running_totals_string_builder_three(numbers):
...
def clamp_string_builder_three(value, low, high):
...
class StringBuilderThreeHelper:
    ...
    def __init__(self, label="string_builder_three"):
        self.label = label
        self._history = []
