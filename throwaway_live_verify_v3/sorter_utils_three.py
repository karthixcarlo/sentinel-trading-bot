"""Throwaway fixture module (71/78) -- not real code, see ../README.md.
...
def normalize_sorter_utils_three(value):
...
def chunk_sorter_utils_three(items, size):
...
def summarize_sorter_utils_three(numbers):
...
def dedupe_preserving_order_sorter_utils_three(items):
...
def flatten_one_level_sorter_utils_three(nested):
...
def running_totals_sorter_utils_three(numbers):
...
def clamp_sorter_utils_three(value, low, high):
...
class SorterUtilsThreeHelper:
    ...
    def __init__(self, label="sorter_utils_three"):
        self.label = label
        self._history = []
