"""Throwaway fixture module (21/78) -- not real code, see ../README.md.
...
def normalize_graph_ops_one(value):
...
def chunk_graph_ops_one(items, size):
...
def summarize_graph_ops_one(numbers):
...
def dedupe_preserving_order_graph_ops_one(items):
...
def flatten_one_level_graph_ops_one(nested):
...
def running_totals_graph_ops_one(numbers):
...
def clamp_graph_ops_one(value, low, high):
...
class GraphOpsOneHelper:
    ...
    def __init__(self, label="graph_ops_one"):
        self.label = label
        self._history = []
