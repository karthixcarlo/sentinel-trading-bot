"""Throwaway fixture module (35/78) -- not real code, see ../README.md.
...
def normalize_json_shape_two(value):
...
def chunk_json_shape_two(items, size):
...
def summarize_json_shape_two(numbers):
...
def dedupe_preserving_order_json_shape_two(items):
...
def flatten_one_level_json_shape_two(nested):
...
def running_totals_json_shape_two(numbers):
...
def clamp_json_shape_two(value, low, high):
...
class JsonShapeTwoHelper:
    ...
    def __init__(self, label="json_shape_two"):
        self.label = label
        self._history = []
