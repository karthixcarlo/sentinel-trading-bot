"""Throwaway fixture module (19/78) -- not real code, see ../README.md.
...
def normalize_sorter_utils_one(value):
...
def chunk_sorter_utils_one(items, size):
...
def summarize_sorter_utils_one(numbers):
...
def dedupe_preserving_order_sorter_utils_one(items):
...
def flatten_one_level_sorter_utils_one(nested):
...
def running_totals_sorter_utils_one(numbers):
...
def clamp_sorter_utils_one(value, low, high):
...
class SorterUtilsOneHelper:
    ...
    def __init__(self, label="sorter_utils_one"):
        self.label = label
        self._history = []
