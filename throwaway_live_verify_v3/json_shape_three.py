"""Throwaway fixture module (61/78) -- not real code, see ../README.md.
...
def normalize_json_shape_three(value):
...
def chunk_json_shape_three(items, size):
...
def summarize_json_shape_three(numbers):
...
def dedupe_preserving_order_json_shape_three(items):
...
def flatten_one_level_json_shape_three(nested):
...
def running_totals_json_shape_three(numbers):
...
def clamp_json_shape_three(value, low, high):
...
class JsonShapeThreeHelper:
    ...
    def __init__(self, label="json_shape_three"):
        self.label = label
        self._history = []
