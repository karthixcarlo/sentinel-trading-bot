"""Throwaway fixture module (46/78) -- not real code, see ../README.md.
...
def normalize_tree_walk_two(value):
...
def chunk_tree_walk_two(items, size):
...
def summarize_tree_walk_two(numbers):
...
def dedupe_preserving_order_tree_walk_two(items):
...
def flatten_one_level_tree_walk_two(nested):
...
def running_totals_tree_walk_two(numbers):
...
def clamp_tree_walk_two(value, low, high):
...
class TreeWalkTwoHelper:
    ...
    def __init__(self, label="tree_walk_two"):
        self.label = label
        self._history = []
