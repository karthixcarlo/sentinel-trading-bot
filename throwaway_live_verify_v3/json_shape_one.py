"""Throwaway fixture module (9/78) -- not real code, see ../README.md.
...
def normalize_json_shape_one(value):
...
def chunk_json_shape_one(items, size):
...
def summarize_json_shape_one(numbers):
...
def dedupe_preserving_order_json_shape_one(items):
...
def flatten_one_level_json_shape_one(nested):
...
def running_totals_json_shape_one(numbers):
...
def clamp_json_shape_one(value, low, high):
...
class JsonShapeOneHelper:
    ...
    def __init__(self, label="json_shape_one"):
        self.label = label
        self._history = []
