"""Throwaway fixture module (73/78) -- not real code, see ../README.md.
...
def normalize_graph_ops_three(value):
...
def chunk_graph_ops_three(items, size):
...
def summarize_graph_ops_three(numbers):
...
def dedupe_preserving_order_graph_ops_three(items):
...
def flatten_one_level_graph_ops_three(nested):
...
def running_totals_graph_ops_three(numbers):
...
def clamp_graph_ops_three(value, low, high):
...
class GraphOpsThreeHelper:
    ...
    def __init__(self, label="graph_ops_three"):
        self.label = label
        self._history = []
