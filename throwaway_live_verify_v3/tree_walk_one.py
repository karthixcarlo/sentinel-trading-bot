"""Throwaway fixture module (20/78) -- not real code, see ../README.md.
...
def normalize_tree_walk_one(value):
...
def chunk_tree_walk_one(items, size):
...
def summarize_tree_walk_one(numbers):
...
def dedupe_preserving_order_tree_walk_one(items):
...
def flatten_one_level_tree_walk_one(nested):
...
def running_totals_tree_walk_one(numbers):
...
def clamp_tree_walk_one(value, low, high):
...
class TreeWalkOneHelper:
    ...
    def __init__(self, label="tree_walk_one"):
        self.label = label
        self._history = []
