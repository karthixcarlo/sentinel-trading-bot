"""Throwaway fixture module (50/78) -- not real code, see ../README.md.
...
def normalize_matrix_ops_two(value):
...
def chunk_matrix_ops_two(items, size):
...
def summarize_matrix_ops_two(numbers):
...
def dedupe_preserving_order_matrix_ops_two(items):
...
def flatten_one_level_matrix_ops_two(nested):
...
def running_totals_matrix_ops_two(numbers):
...
def clamp_matrix_ops_two(value, low, high):
...
class MatrixOpsTwoHelper:
    ...
    def __init__(self, label="matrix_ops_two"):
        self.label = label
        self._history = []
