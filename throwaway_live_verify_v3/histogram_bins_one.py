"""Throwaway fixture module (26/78) -- not real code, see ../README.md.
...
def normalize_histogram_bins_one(value):
...
def chunk_histogram_bins_one(items, size):
...
def summarize_histogram_bins_one(numbers):
...
def dedupe_preserving_order_histogram_bins_one(items):
...
def flatten_one_level_histogram_bins_one(nested):
...
def running_totals_histogram_bins_one(numbers):
...
def clamp_histogram_bins_one(value, low, high):
...
class HistogramBinsOneHelper:
    ...
    def __init__(self, label="histogram_bins_one"):
        self.label = label
        self._history = []
