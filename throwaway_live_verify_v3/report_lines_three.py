"""Throwaway fixture module (58/78) -- not real code, see ../README.md.
...
def normalize_report_lines_three(value):
...
def chunk_report_lines_three(items, size):
...
def summarize_report_lines_three(numbers):
...
def dedupe_preserving_order_report_lines_three(items):
...
def flatten_one_level_report_lines_three(nested):
...
def running_totals_report_lines_three(numbers):
...
def clamp_report_lines_three(value, low, high):
...
class ReportLinesThreeHelper:
    ...
    def __init__(self, label="report_lines_three"):
        self.label = label
        self._history = []
