"""Throwaway fixture module (63/78) -- not real code, see ../README.md.
...
def normalize_cache_lite_three(value):
...
def chunk_cache_lite_three(items, size):
...
def summarize_cache_lite_three(numbers):
...
def dedupe_preserving_order_cache_lite_three(items):
...
def flatten_one_level_cache_lite_three(nested):
...
def running_totals_cache_lite_three(numbers):
...
def clamp_cache_lite_three(value, low, high):
...
class CacheLiteThreeHelper:
    ...
    def __init__(self, label="cache_lite_three"):
        self.label = label
        self._history = []
