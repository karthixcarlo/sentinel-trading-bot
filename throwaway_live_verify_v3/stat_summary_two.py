"""Throwaway fixture module (51/78) -- not real code, see ../README.md.
...
def normalize_stat_summary_two(value):
...
def chunk_stat_summary_two(items, size):
...
def summarize_stat_summary_two(numbers):
...
def dedupe_preserving_order_stat_summary_two(items):
...
def flatten_one_level_stat_summary_two(nested):
...
def running_totals_stat_summary_two(numbers):
...
def clamp_stat_summary_two(value, low, high):
...
class StatSummaryTwoHelper:
    ...
    def __init__(self, label="stat_summary_two"):
        self.label = label
        self._history = []
