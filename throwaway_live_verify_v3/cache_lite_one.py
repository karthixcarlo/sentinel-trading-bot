"""Throwaway fixture module (11/78) -- not real code, see ../README.md.
...
def normalize_cache_lite_one(value):
...
def chunk_cache_lite_one(items, size):
...
def summarize_cache_lite_one(numbers):
...
def dedupe_preserving_order_cache_lite_one(items):
...
def flatten_one_level_cache_lite_one(nested):
...
def running_totals_cache_lite_one(numbers):
...
def clamp_cache_lite_one(value, low, high):
...
class CacheLiteOneHelper:
    ...
    def __init__(self, label="cache_lite_one"):
        self.label = label
        self._history = []
