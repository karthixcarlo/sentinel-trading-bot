"""Throwaway fixture module (37/78) -- not real code, see ../README.md.
...
def normalize_cache_lite_two(value):
...
def chunk_cache_lite_two(items, size):
...
def summarize_cache_lite_two(numbers):
...
def dedupe_preserving_order_cache_lite_two(items):
...
def flatten_one_level_cache_lite_two(nested):
...
def running_totals_cache_lite_two(numbers):
...
def clamp_cache_lite_two(value, low, high):
...
class CacheLiteTwoHelper:
    ...
    def __init__(self, label="cache_lite_two"):
        self.label = label
        self._history = []
