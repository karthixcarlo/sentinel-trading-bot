"""Throwaway fixture module (6/78) -- not real code, see ../README.md.
...
def normalize_report_lines_one(value):
...
def chunk_report_lines_one(items, size):
...
def summarize_report_lines_one(numbers):
...
def dedupe_preserving_order_report_lines_one(items):
...
def flatten_one_level_report_lines_one(nested):
...
def running_totals_report_lines_one(numbers):
...
def clamp_report_lines_one(value, low, high):
...
class ReportLinesOneHelper:
    ...
    def __init__(self, label="report_lines_one"):
        self.label = label
        self._history = []
