"""Throwaway fixture module (70/78) -- not real code, see ../README.md.
...
def normalize_unit_convert_three(value):
...
def chunk_unit_convert_three(items, size):
...
def summarize_unit_convert_three(numbers):
...
def dedupe_preserving_order_unit_convert_three(items):
...
def flatten_one_level_unit_convert_three(nested):
...
def running_totals_unit_convert_three(numbers):
...
def clamp_unit_convert_three(value, low, high):
...
class UnitConvertThreeHelper:
    ...
    def __init__(self, label="unit_convert_three"):
        self.label = label
        self._history = []
