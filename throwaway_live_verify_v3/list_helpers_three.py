"""Throwaway fixture module (54/78) -- not real code, see ../README.md.
...
def normalize_list_helpers_three(value):
...
def chunk_list_helpers_three(items, size):
...
def summarize_list_helpers_three(numbers):
...
def dedupe_preserving_order_list_helpers_three(items):
...
def flatten_one_level_list_helpers_three(nested):
...
def running_totals_list_helpers_three(numbers):
...
def clamp_list_helpers_three(value, low, high):
...
class ListHelpersThreeHelper:
    ...
    def __init__(self, label="list_helpers_three"):
        self.label = label
        self._history = []
