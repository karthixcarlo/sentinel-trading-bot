"""Throwaway fixture module (27/78) -- not real code, see ../README.md.
...
def normalize_text_formatting_two(value):
...
def chunk_text_formatting_two(items, size):
...
def summarize_text_formatting_two(numbers):
...
def dedupe_preserving_order_text_formatting_two(items):
...
def flatten_one_level_text_formatting_two(nested):
...
def running_totals_text_formatting_two(numbers):
...
def clamp_text_formatting_two(value, low, high):
...
class TextFormattingTwoHelper:
    ...
    def __init__(self, label="text_formatting_two"):
        self.label = label
        self._history = []
