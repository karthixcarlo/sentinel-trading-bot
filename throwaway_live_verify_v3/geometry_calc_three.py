"""Throwaway fixture module (69/78) -- not real code, see ../README.md.
...
def normalize_geometry_calc_three(value):
...
def chunk_geometry_calc_three(items, size):
...
def summarize_geometry_calc_three(numbers):
...
def dedupe_preserving_order_geometry_calc_three(items):
...
def flatten_one_level_geometry_calc_three(nested):
...
def running_totals_geometry_calc_three(numbers):
...
def clamp_geometry_calc_three(value, low, high):
...
class GeometryCalcThreeHelper:
    ...
    def __init__(self, label="geometry_calc_three"):
        self.label = label
        self._history = []
