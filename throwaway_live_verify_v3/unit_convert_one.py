"""Throwaway fixture module (18/78) -- not real code, see ../README.md.
...
def normalize_unit_convert_one(value):
...
def chunk_unit_convert_one(items, size):
...
def summarize_unit_convert_one(numbers):
...
def dedupe_preserving_order_unit_convert_one(items):
...
def flatten_one_level_unit_convert_one(nested):
...
def running_totals_unit_convert_one(numbers):
...
def clamp_unit_convert_one(value, low, high):
...
class UnitConvertOneHelper:
    ...
    def __init__(self, label="unit_convert_one"):
        self.label = label
        self._history = []
