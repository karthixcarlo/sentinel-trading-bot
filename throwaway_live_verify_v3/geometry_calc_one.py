"""Throwaway fixture module (17/78) -- not real code, see ../README.md.
...
def normalize_geometry_calc_one(value):
...
def chunk_geometry_calc_one(items, size):
...
def summarize_geometry_calc_one(numbers):
...
def dedupe_preserving_order_geometry_calc_one(items):
...
def flatten_one_level_geometry_calc_one(nested):
...
def running_totals_geometry_calc_one(numbers):
...
def clamp_geometry_calc_one(value, low, high):
...
class GeometryCalcOneHelper:
    ...
    def __init__(self, label="geometry_calc_one"):
        self.label = label
        self._history = []
