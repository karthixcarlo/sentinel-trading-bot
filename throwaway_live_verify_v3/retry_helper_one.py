"""Throwaway fixture module (10/78) -- not real code, see ../README.md.
...
def normalize_retry_helper_one(value):
...
def chunk_retry_helper_one(items, size):
...
def summarize_retry_helper_one(numbers):
...
def dedupe_preserving_order_retry_helper_one(items):
...
def flatten_one_level_retry_helper_one(nested):
...
def running_totals_retry_helper_one(numbers):
...
def clamp_retry_helper_one(value, low, high):
...
class RetryHelperOneHelper:
    ...
    def __init__(self, label="retry_helper_one"):
        self.label = label
        self._history = []
