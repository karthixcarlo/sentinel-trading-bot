...
with col3:
    ...
    if 9 <= current_time.hour < 16 and current_time.weekday() < 5:
        market_status = ":material/radio_button_checked: OPEN"
        status_color = "#00D09C"
    else:
        market_status = ":material/radio_button_unchecked: CLOSED"
        status_color = "#EB5B3C"
...
Use the **top navigation** to explore different features:

- **:material/bar_chart: Market Overview** - View market indices, watchlist, and portfolio summary
- **:material/search: Stock Discovery** - Find top gainers, losers, and most active stocks
- **:material/analytics: Analyzer** - Deep analysis with BUY/SELL signals and charts
- **:material/account_balance_wallet: Portfolio** - Track your holdings, P&L, and order history
- **:material/bolt: Trade Executor** - Place buy/sell orders with validation
- **:material/settings: Settings** - Configure your preferences
...
st.subheader(":material/account_balance_wallet: Portfolio Snapshot")
...
with col1:
    st.subheader(":material/bookmark: Watchlist")
...
with col2:
    ...
    if recent_orders:
        for order in reversed(recent_orders):
            ...
            st.markdown(f"""
            <div style='margin-bottom: 8px; padding: 8px; background-color: #F6F8FA; border-radius: 6px;'>
    ...
    if st.button(":material/bolt: Place Order", use_container_width=True, icon=":material/bolt:"):
        st.switch_page("pages/Trade_Executor.py")
...
st.markdown("---")
st.subheader(":material/widgets: Quick Actions")

col1, col2, col3, col4 = st.columns(4)

with col1:
    if st.button("Discover Stocks", use_container_width=True, type="primary", icon=":material/search:"):
        st.switch_page("pages/2_🔍_Stock_Discovery.py")

with col2:
    if st.button("Analyze Stock", use_container_width=True, icon=":material/analytics:"):
        st.switch_page("pages/3_📈_Stock_Analyzer.py")

with col3:
    if st.button("View Portfolio", use_container_width=True, icon=":material/account_balance_wallet:"):
        st.switch_page("pages/4_💼_Portfolio.py")

with col4:
    if st.button("Execute Trade", use_container_width=True, icon=":material/bolt:"):
...
st.markdown("---")
st.caption(":material/info: Tip: Use the top navigation to switch between pages. All trading is simulated (paper trading).")
