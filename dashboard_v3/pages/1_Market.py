...
# Page setup
setup_page_config("Market Overview", "📊")
...
with col1:
    with st.container(border=True):
        ...
        if st.button("Discover Now", use_container_width=True, key="action_discover"):
...
with col2:
    with st.container(border=True):
        ...
        if st.button("Analyze Now", use_container_width=True, key="action_analyze"):
...
with col3:
    with st.container(border=True):
        ...
        if st.button("Trade Now", use_container_width=True, key="action_trade"):
...
if positions:
    ...
    st.info("No positions yet. Start trading to see your holdings here!", icon="ℹ️")
...
if is_open:
    st.success("Market is OPEN - Trading hours: 9:15 AM - 3:30 PM IST", icon="✅")
else:
    st.info("Market is CLOSED - Opens tomorrow at 9:15 AM IST", icon="🕒")
