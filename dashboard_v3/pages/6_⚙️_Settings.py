"""
⚙️ Settings - Multi-Page Dashboard
...
with st.form("settings_form"):
    ...
    if st.form_submit_button("💾 Save Settings", type="primary"):
        ...
        st.success(":material/check: Settings saved!")
        st.toast("Settings updated", icon=":material/check:")
...
with col1:
    ...
    if uploaded:
        try:
            ...
            st.success(f":material/check: Imported {len(imported_stocks)} stocks!")
            st.rerun()
...
# Reset portfolio
st.subheader(":material/refresh: Reset Portfolio")

st.warning("⚠️ This will reset your portfolio to initial state and delete all positions and orders!")
...
if st.button("Reset Portfolio", type="secondary"):
    ...
    st.success(":material/check: Portfolio reset!")
    st.toast("Portfolio reset to initial state", icon=":material/refresh:")
    st.rerun()
