...
st.set_page_config(page_title="Stock Analyzer", page_icon="📈", layout="wide", initial_sidebar_state="collapsed")
...
st.title(":material/show_chart: Stock Analyzer")
...
if not symbol:
    st.info("Select a stock to analyze", icon=":material/info:")
    st.stop()
...
if not data or data['history'].empty:
    st.error("No data found for this stock", icon=":material/error:")
    st.stop()
...
st.subheader(":material/candlestick_chart: Price Chart with Indicators")
...
st.markdown("---")
st.subheader(":material/psychology: AI Analyst Insight")
...
if GEMINI_AVAILABLE:
    ...
    with col_news:
        st.markdown("### :material/newspaper: Recent News")
    ...
    with col_ai:
        st.markdown("### :material/smart_toy: Gemini AI Analysis")
        
        with st.spinner(":material/analytics: Gemini is analyzing market sentiment & technicals..."):
            try:
                # Initialize analyst
                ...
                if signal.signal == "BUY":
                    ...
                    <div style='background: linear-gradient(135deg, #00D09C 0%, #00B386 100%); 
                        ...
                        <h2 style='margin:0; color: white;'>
                            <svg style='vertical-align: middle; margin-right: 8px;' width='24' height='24' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'>
...
st.subheader(":material/bolt: Quick Trade")
...
with st.form("quick_trade_form"):
    ...
    if submitted:
        ...
        if not market_open:
            st.error(f":material/schedule: {market_msg}")
            st.info("Orders can only be placed during market hours: **Monday-Friday, 9:15 AM - 3:30 PM IST**", icon=":material/info:")
        else:
            ...
            if side == "BUY" and cash < total_cost:
                st.error(f":material/error: Insufficient funds! Available: ₹{cash:,.2f}, Required: ₹{total_cost:,.2f}")
            elif side == "SELL":
                ...
                if not existing_pos or existing_pos.get('quantity', 0) < quantity:
                    st.error(f":material/error: Insufficient shares! You need {quantity} shares to sell.")
                else:
                    # Execute sell
                    ...
                    st.success(f":material/check_circle: SOLD {quantity} shares of {symbol.replace('.NS', '')} @ ₹{limit_price:,.2f}")
                    st.balloons()
                    st.toast(f"Order placed: SELL {quantity} shares", icon=":material/check:")
            else:
                # Execute buy
                ...
                st.success(f":material/check_circle: BOUGHT {quantity} shares of {symbol.replace('.NS', '')} @ ₹{limit_price:,.2f}")
                st.balloons()
                st.toast(f"Order placed: BUY {quantity} shares", icon=":material/check:")
