        ...
        if not symbol:
            ...
            if side == "BUY":
                if cash < final_cost:
                    ...
                    st.success(f"Order placed: BUY {quantity} shares of {symbol} @ ₹{limit_price:,.2f}")
                    st.balloons()
                ...
                if not existing_pos:
                    ...
                    st.success(f"Order placed: SELL {quantity} shares of {symbol} @ ₹{limit_price:,.2f}")
                    st.balloons()
                    st.toast(f"Sold {quantity} shares of {symbol}")

st.markdown("---")
st.caption("All trades are simulated (paper trading). No real money is involved.")
